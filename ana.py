...
def simulate():
    ...
    for ticker in tqdm(tickers, ncols=80, bar_format='{percentage:3.0f}%|{bar}{r_bar}'):
        cache_name = os.path.join(dir_path, 'cache', 'cache-%s.json' % (ticker,))
    ...
    for cutoff in range(start_point - 1, sl - 1):
        ...
        buy_symbols = []
        for ticker, series in tqdm(serieses.items(), ncols=80, bar_format='{percentage:3.0f}%|{bar}{r_bar}',
                                   leave=False):
